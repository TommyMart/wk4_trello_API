...
# from flask_jwt_extended import jwt_required, flask_jwt_extended
...
comments_bp = Blueprint("comments", __name__, url_prefix="/cards")

# we already get the comments while fetching cards, so, no need for
# get comments route here 
...
#             date=date.today(),
#             card=card.id,
#             # only user id str(user.id) logged in user id
#             user=get_jwt_identity
#         )
#         # add and commit the session
#         db.session.add(comment)
#         db.session.commit()
#         # return the create commit 
#         return comment_schema.dump(comment), 201
#     # else
#     else:
#         # return an error like card does not exist 
